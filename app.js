...
async function downloadOffline(city,district,status,bar){
  ...
  // ★ 1712 修正：每一個行政區建立自己的 Cache。
  // 以前所有行政區都共用 geolive-tiles-z15-v1，
  // 因此不同地區的圖磚會混在一起，看起來像下載相同範圍。
...
async function migrateOfflineCache1712(){
  const flag='geolive_offline_cache_migrated_1712';
  if(localStorage.getItem(flag)==='1')return;

  // 1711 以前把不同行政區存到同一個 cache，無法可靠分離。
  // 只清除離線地圖資料，不動專案、照片、GPX 等資料。
  await caches.delete(LEGACY_TILE_CACHE);
  const oldMaps=await dbAll('offline');
  for(const m of oldMaps){
    if(m?.cache_name)await caches.delete(m.cache_name);
    await dbDel('offline',m.id);
  }
...
(async()=>{try{initMap();await migrateOfflineCache1712();await loadProjects();startWatch();updateNetwork();drawProfile([])}catch(e){toast('初始化失敗：'+e.message,5000)}})();
