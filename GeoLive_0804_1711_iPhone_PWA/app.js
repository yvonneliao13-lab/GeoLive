...
async function resolveRegionPolygon(city,district){
  const q=encodeURIComponent(`${district}, ${city}, Taiwan`);
  const url=`https://nominatim.openstreetmap.org/search?q=${q}&format=jsonv2&countrycodes=tw&limit=10&polygon_geojson=1&addressdetails=1`;
  const r=await fetch(url,{headers:{Accept:'application/json','Accept-Language':'zh-TW'}});
  if(!r.ok)throw new Error('行政區邊界查詢失敗');
  const rows=await r.json();

  // 不再接受 rows[0] 這種模糊結果；必須同時符合縣市與鄉鎮市區名稱。
  const hit=rows.find(x=>{
    const a=x.address||{};
    const display=x.display_name||'';
    const countyOk =
      a.city===city || a.county===city || a.state===city ||
      display.includes(city);
    const townValues=[a.city_district,a.town,a.suburb,a.municipality,a.village].filter(Boolean);
    const townOk=townValues.includes(district) || display.includes(district);
    const geometryOk=x.geojson && (x.geojson.type==='Polygon'||x.geojson.type==='MultiPolygon');
    return countyOk && townOk && geometryOk;
  });
  if(!hit)throw new Error(`找不到完全符合「${city} ${district}」的行政區邊界，已停止下載，避免抓錯範圍。`);

  const bb=(hit.boundingbox||[]).map(Number);
  if(bb.length!==4||bb.some(v=>!Number.isFinite(v)))throw new Error('行政區邊界資料不完整');
  const [south,north,west,east]=bb;
  return {
    bounds:[west,south,east,north],
    geometry:hit.geojson,
    matchedName:hit.display_name||`${city}${district}`
  };
}
...
async function downloadOffline(city,district,status,bar){
  ...
  // 只下載中心點落在行政區 Polygon 內的圖磚，不再下載整個 bounding box。
  const jobs=[];
  for(let x=x1;x<=x2;x++){
    for(let y=y1;y<=y2;y++){
  ...
  if(expected<1)throw new Error('行政區內沒有可下載的 Zoom 15 圖磚');
  if(expected>15000)throw new Error(`校正後範圍仍需要 ${expected} 張圖磚，超過 15000 張安全上限`);

  status.textContent=`已確認：${region.matchedName}｜共 ${expected} 張道路圖磚`;
  const cache=await caches.open(TILE_CACHE);
  let done=0,errors=0;

  for(let i=0;i<jobs.length;i+=6){
    await Promise.all(jobs.slice(i,i+6).map(async([x,y])=>{
      ...
      status.textContent=`${city}${district}｜道路圖下載 ${done}/${expected}｜失敗 ${errors}`;
      bar.style.width=`${done/expected*100}%`;
    }));
  }

  const id=`${Date.now()}_${safeName(city+district)}`;
  const m={
    id,label:city+district,city,district,bounds,geometry,
...
async function toggleOffline(id){if(state.offlineLayers.has(id)){map.removeLayer(state.offlineLayers.get(id));state.offlineLayers.delete(id);return}const m=await dbGet('offline',id);if(!m)return;const l=L.tileLayer(TILE_URL,{minNativeZoom:15,maxNativeZoom:15,minZoom:2,maxZoom:20,bounds:[[m.bounds[1],m.bounds[0]],[m.bounds[3],m.bounds[2]]]}).addTo(map);state.offlineLayers.set(id,l);map.fitBounds([[m.bounds[1],m.bounds[0]],[m.bounds[3],m.bounds[2]]])}
async function deleteOffline(id){if(state.offlineLayers.has(id))map.removeLayer(state.offlineLayers.get(id));state.offlineLayers.delete(id);await dbDel('offline',id);toast('已刪除離線地圖清單；圖磚快取將由清理功能移除')}
async function clearTileCache(){await caches.delete(TILE_CACHE);toast('已清除全部離線圖磚快取')}
...
$('#offlineDownloadBtn').onclick=()=>{const cities=Object.keys(DISTRICTS).map(x=>`<option>${x}</option>`).join('');openModal('下載離線道路地圖（行政區校正版）',`<div class="form-row"><label>縣市</label><select id="citySel">${cities}</select></div><div class="form-row"><label>區／鄉鎮市</label><select id="districtSel"></select></div><button id="doDownload" class="primary">確認行政區後下載 Zoom 15</button><div class="progress"><div id="dlBar"></div></div><p id="dlStatus"></p><small class="note">下載期間請保持 GeoLive 在前景並關閉自動鎖定。iPhone 儲存空間不足時可能失敗。</small>`);const fill=()=>{$('#districtSel').innerHTML=DISTRICTS[$('#citySel').value].map(x=>`<option>${x}</option>`).join('')};fill();$('#citySel').onchange=fill;$('#doDownload').onclick=async()=>{const s=$('#dlStatus'),bar=$('#dlBar');s.textContent='正在取得行政區範圍…';try{const m=await downloadOffline($('#citySel').value,$('#districtSel').value,s,bar);s.textContent=`完成：${m.tiles}/${m.expected_tiles} 張，失敗 ${m.errors}`;toast('行政區範圍校正完成，離線道路地圖已下載')}catch(e){s.textContent='失敗：'+e.message}}};
...
(async()=>{try{initMap();await loadProjects();startWatch();updateNetwork();drawProfile([])}catch(e){toast('初始化失敗：'+e.message,5000)}})();
